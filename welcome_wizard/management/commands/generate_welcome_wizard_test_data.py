"""Generate test data for the Welcome Wizard app."""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--flush",
            action="store_true",
            help="Flush any existing Welcome Wizard test data from the database before generating new data.",
        )

    def _generate_static_data(self, db):
        """Generate static data required for test cases."""
        # TODO: populate the database with Welcome Wizard test data.

    def handle(self, *args, **options):
        """Entry point to the management command."""
        if options["flush"]:
            self.stdout.write("Flushing existing Welcome Wizard test data...")
