...
NAME = os.getenv("IMAGE_NAME", "merlin")
PWD = os.getcwd()
...
def unittest(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER, keepdb=False, verbosity=1):
    ...
    command = f"nautobot-server test merlin --verbosity={verbosity}"
    if keepdb:
        command += " --keepdb"
...
def pylint(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f"{docker} sh -c \"cd /source && find . -name '*.py' -not -path '*/migrations/*' -not -path '*/tests/*' -not -path '*/development/*' | "
        'PYTHONPATH=/opt/nautobot/nautobot xargs pylint"',
        env={"NAUTOBOT_VER": nautobot_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
...
def pydocstyle(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    # We exclude the /migrations/ directory since it is autogenerated code
    context.run(
        f"{docker} sh -c \"cd /source && find . -name '*.py' -not -path '*/migrations/*' -not -path '*/tests/*' -not -path '*/development/*' | xargs pydocstyle\"",
        env={"NAUTOBOT_VER": nautobot_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
