...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.welcome_wizard.compose_files
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.welcome_wizard.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.welcome_wizard.compose_dir, "creds.example.env"
            ),
            os.path.join(context.welcome_wizard.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.welcome_wizard.python_ver
    dockerfile_path = os.path.join(context.welcome_wizard.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.welcome_wizard.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'welcome_wizard'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("welcome_wizard")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "welcome_wizard.migrations"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Welcome Wizard."""
    command = "nautobot-server generate_welcome_wizard_test_data"
    if database:
