...
NAUTOBOT_VER = os.getenv("NAUTOBOT_VER", "v1.0.0b4")
...
def destroy(context):
    ...
    docker_compose(context, "down")
    context.run(f"docker volume rm -f {BUILD_NAME}_pgdata_merlin")
...
def unittest(context, keepdb=False, verbosity=1):
    ...
    entrypoint = f"nautobot-server test merlin --verbosity={verbosity}"
    if keepdb:
        entrypoint += " --keepdb"
    command = f"run --entrypoint '{entrypoint}' nautobot"
