...
class PlaceholderAPITest(TestCase):
    ...
    def test_device_type_import_api(self):
        ...
        url = reverse("plugins-api:merlin-api:devicetypeimport-list")
    ...
    def test_manufacturer_import_api(self):
        ...
        url = reverse("plugins-api:merlin-api:manufacturerimport-list")
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 0)
    ...
    def test_devicetype_with_data(self):
        ...
        url = reverse("plugins-api:merlin-api:devicetypeimport-detail", kwargs={"pk": devicetype.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_manufacturer_with_data(self):
        ...
        url = reverse("plugins-api:merlin-api:manufacturerimport-detail", kwargs={"pk": manufacturer.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
